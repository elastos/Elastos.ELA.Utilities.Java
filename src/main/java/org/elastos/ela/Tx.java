...
public class Tx {
    ...
    private PayloadRegisterProducer payloadRegisterProducer;
    private PayloadUpdateProducer payloadUpdateProducer;
    ...
    // dpos
    final static byte REGISTER_PRODUCER = 0x09;
    public final static byte CANCEL_PRODUCER = 0x0a;
    final static byte UPDATE_PRODUCER = 0x0b;
    final static byte RETURN_PLEDGECOIN = 0x0c;
    public final static byte ACTIVATE_PRODUCER = 0x0d;
    ...
    //Register producer
    public static Tx registerProducerTransaction(byte TransactionType, UTXOTxInput[] inputs, TxOutput[] outputs, PayloadRegisterProducer payloadRegisterProducer) {
        Tx tx = new Tx();
        tx.payloadRegisterProducer = payloadRegisterProducer;
        commonalityTransaction(tx, TransactionType, inputs, outputs);
        return tx;
    }

    //Update producer
    public static Tx updateProducerTransaction(byte TransactionType, UTXOTxInput[] inputs, TxOutput[] outputs, PayloadUpdateProducer updateProducer) {
        Tx tx = new Tx();
        tx.payloadUpdateProducer = updateProducer;
        commonalityTransaction(tx, TransactionType, inputs, outputs);
        return tx;
    }

    //Cancel producer
    public static Tx processProducerTransaction(byte TransactionType, UTXOTxInput[] inputs, TxOutput[] outputs, ProcessProducer processProducer) {
        Tx tx = new Tx();
        tx.processProducer = processProducer;
        commonalityTransaction(tx, TransactionType, inputs, outputs);
        return tx;
    }
        ...
        switch (this.TxType) {
            ...
            case TRANSFER_CROSS_CHAIN_ASSET:
                ...
                for (PayloadTransferCrossChainAsset ca : this.CrossChainAsset)
                    ca.Serialize(o);
                break;
            // INVOKE and REGISTER_PRODUCER conflict
//            case INVOKE:
//                this.payloadInvoke.Serialize(o);
//                break;
            case REGISTER_PRODUCER:
                this.payloadRegisterProducer.Serialize(o);
                break;
            case CANCEL_PRODUCER:
                this.processProducer.Serialize(o);
                break;
            case UPDATE_PRODUCER:
                this.payloadUpdateProducer.Serialize(o);
                break;
            case RETURN_PLEDGECOIN:
                break;
            case ACTIVATE_PRODUCER:
                this.processProducer.Serialize(o);
        }
    }
}
