# Web Request

 - start command : java -cp Elastos.ELA.Utilities.Java.jar  org.elastos.elaweb.HttpServer
 - suggestion：java version "1.8.0_161"
 - local IP : http://127.0.0.1:8989/

## genrawtx
description:constructing single sign transaction

#### Tips :

- You need to calculate the zero-finding address balance, which is equal to input-outputs-fee, and write the zero-finding address and balance on the last line of outputs.

#### Parameter Description

| name  | type | description |
| ------ | ---- | ----------- |
| txid | string | transaction where the available balance of address is located |
| vout| int | the serial number of the tx in which the balance is available 
| privatekey | string | Utxo address corresponds to the private key| 
| address | string | arrival address | 
| amount | string | the unit is ELA,Namely:200.01ELA | 


##### Request
```
{
    "method":"genrawtx",
    "params":{
        "transaction":{
            "inputs":[
                {
                    "txid":"28cb087f7feabb9ff2fc34fb541946e4052e7d0aba50ebbc0773a69c9ca952fe",
                    "vout":0,
                    "privatekey":"5FA927E5664E563F019F50DCD4D7E2D9404F2D5D49E31F9482912E23D6D7B9EB"
                }
            ],
            "outputs":[
                {
                    "address":"ENvYuRCpY42hX8gYAcdbwiD2GNZmUWWSix",
                    "amount":"2"
                },
                {
                    "address":"ENvYuRCpY42hX8gYAcdbwiD2GNZmUWWSix",
                    "amount":"2"
                }
            ]
        }
    }
}
```

##### Response
```
{
    "result": {
        "rawtx": "",
        "txhash": "4382093E71F362AF07E3D5157503E687BBA007D3A974EC27A6A832C785B4D2B0"
    },
    "id": null,
    "jsonrpc": "2.0",
    "error": null
}

```

## genrawtxbyprivatekey

#### Tips：
- Do not calculate utxo
- No need to calculate the amount of the change address，Get the utxo sorted from small to large, and automatically calculate the change amount according to the output amount.
- java-config.json files need to be placed in the same level directory of the Java program to connect nodes to get utxo

#### Parameter Description

| name  | type | description |
| ------ | ---- | ----------- |
| Host| string | Server IP and RPC ports where the node program resides | 
| Fee| string | The single output or multiple input and output fees of a transaction are the same | 
| Confirmation| string | Number of block confirmation transactions | 
| privatekey | string | Utxo address corresponds to the private key，Automatic acquisition of utxo| 
| address | string | arrival address | 
| amount | string | the unit is ELA,Namely:289.1ELA | 
| changeaddress | string | Change address | 

#### 接口名：genrawtxbyprivatekey

- ##### java-config.json

```
{
  "Host": "127.0.0.1:11336",
  "Fee":"0.0005",
  "Confirmation":16
}

```

##### Request

```
{
    "method":"genrawtxbyprivatekey",
    "params":{
        "transaction":{
            "privatekeys":[
                {
                    "privatekey":"5FA927E5664E563F019F50DCD4D7E2D9404F2D5D49E31F9482912E23D6D7B9EB"
                }
            ],
            "outputs":[
                {
                    "address":"Eazj14ifau5eH1SP5F8MJRuiSsPMiGbJV1",
                    "amount":"0.01"
                }
            ],
            "changeaddress":"Eazj14ifau5eH1SP5F8MJRuiSsPMiGbJV1"
        }
    }
}

```

##### Response

```
{
    "result": {
        "rawtx": "",
        "txhash": "CE0285BC67E040057D5EBC4B3E14D02DC2326DA8ABB2D209965EE5D8463812A7"
    },
    "id": null,
    "jsonrpc": "2.0",
    "error": null
}
```



## genmultisigntx
description:multi sign transaction

#### Parameter Description

| name  | type | description |
| ------ | ---- | ----------- |
| txid | string | transaction where the available balance of address is located |
| vout| int | the serial number of the tx in which the balance is available  
| address | string | multi sign address| 
| amount | string | the unit is ELA,Namely:200.01ELA | 
| privatekeyscripte |  |private key for generating multi-signature addresses | 
| m | int | sign number | 

##### Request

```
      
{
    "method":"genmultisigntx",
    "params":{
        "transaction":{
            "inputs":[
                {
                    "txid":"02d79aeae97d879f850e0c7ebb9bb62d93d1fbe4665d8e895106636e9e2656ae",
                    "vout":1,
                    "address":"8ZNizBf4KhhPjeJRGpox6rPcHE5Np6tFx3"
                }
            ],
            "outputs":[
                {
                    "address":"ERz34iKa4nGaGYVtVpRWQZnbavJEe6PRDt",
                    "amount":"20"
                }
            ],
            "privatekeyscripte":[
                {
                    "privatekey":"927f1ff719047e0243150447b9c009fc2f17d67fd413beb965b9a9449d42b9b1"
                },
                {
                    "privatekey":"8d57d983f5960f6b3b2ed1d4f7350cfa7fb985580eaf4b9a2d8501384ce27369"
                },
                {
                    "privatekey":"eebfbd55819ea095107b776616669536d9f0bb6d3cd9b21665ea1fb02405cfed"
                },
                {
                    "privatekey":"22e388e026234863ba077fe18783bbf7935c49ed08898995e7f5f64db8d51cef"
                }
            ],
        	"m":3
        }
    }
}

```

##### Response

```
{
    "result": {
        "rawtx": "",
        "txhash": "4C1D416975650CB03228694116FBF5937BD6458E0F846B0843FF685868465DE5"
    },
    "id": null,
    "jsonrpc": "2.0",
    "error": null
}
}
```

## gencrosschaintx
description:cross chain single sign transcation

#### Tips：
- recharge：The x address is generated by hash in the Genesis Block of the Side Chain
- withdraw：address = "0000000000000000000000000000000000"

#### Parameter Description
| name  | type | description |
| ------ | ---- | ----------- |
| txid | string | transaction where the available balance of address is located |
| vout| int | the serial number of the tx in which the balance is available 
| privatekeysign |  |Private key corresponding to the address of inputs | 
| crosschainasset |  |Side Chain to Account Address and Balance | 

##### Request
```
{
    "method":"gencrosschaintx",
    "params":{
	    "transaction":{
            "inputs":[
                {
                    "txid":"4bb237fb16a233dbd2414a014da7ca02c215bb6ffd976acc84b5cf4ec0439ba0",
                    "vout":0,
                    "address":"Eco3fa2sZfyY6P524JTK9WpvzFjKEaoF5U"
                }
            ],
            "outputs":[
                {
                    "address":"XKUh4GLhFJiqAMTF6HyWQrV9pK9HcGUdfJ",
                    "amount":"0.00039899"
                }
            ],
            "privatekeysign":[
                {
                    "privatekey":"E6710E4FC7E8057BDEE6E129949A70AACE72A0EA0722E7B90E41765D6C1C407E"
                }
            ],
            "crosschainasset":[
                {
                    "address":"Eco3fa2sZfyY6P524JTK9WpvzFjKEaoF5U",
                    "amount":"0.00029899"
                }
            ]
        }
	}
}


```

##### Response
```
{
    "result": {
        "rawtx": "",
        "txhash": "F9F961F5BCA14F8C005CA4A5B3334B802CE719C4DDA8504CC99BD83B744EEE9E"
    },
    "id": null,
    "jsonrpc": "2.0",
    "error": null
}
```


## gencrosschainmultisigntx
description:cross chain multi sign transcation

#### Tips：
- recharge：The x address is generated by hash in the Genesis Block of the Side Chain
- withdraw：address = "0000000000000000000000000000000000"

#### Parameter Description
| name  | type | description |
| ------ | ---- | ----------- |
| txid | string | transaction where the available balance of address is located |
| vout| int | the serial number of the tx in which the balance is available 
| crosschainasset |  |Side Chain to Account Address and Balance | 
| privatekeyscripte |  |private key for generating multi-signature addresses | 
| m | int | sign number | 

##### Request
```
{
    "method":"gencrosschainmultisigntx",
    "params":{
        "transaction":{
            "inputs":[
                {
                    "txid":"723523b9f92394b9428c7abf4d13d7049efeba3c3a154e1b88e96ff6b95bd608",
                    "vout":0,
                    "address":"8NRxtbMKScEWzW8gmPDGUZ8LSzm688nkZZ"
                }
            ],
            "outputs":[
                {
                    "address":"XKUh4GLhFJiqAMTF6HyWQrV9pK9HcGUdfJ",
                    "amount":"0.3"
                }
            ],
            "privatekeyscripte":[
                {
                    "privatekey":"927f1ff719047e0243150447b9c009fc2f17d67fd413beb965b9a9449d42b9b1"
                },
                {
                    "privatekey":"22e388e026234863ba077fe18783bbf7935c49ed08898995e7f5f64db8d51cef"
                },
                {
                    "privatekey":"8d57d983f5960f6b3b2ed1d4f7350cfa7fb985580eaf4b9a2d8501384ce27369"
                },
                {
                    "privatekey":"22e388e026234863ba077fe18783bbf7935c49ed08898995e7f5f64db8d51cef"
                }
            ],
            "crosschainasset":[
                {
                    "address":"EQSpUzE4XYJhBSx5j7Tf2cteaKdFdixfVB",
                    "amount":"0.3"
                }
            ],
        	"m":3
        }
    }
}

```

##### Response
```
{
    "result": {
        "rawtx": "",
        "txhash": "AB23BE6A44CFE3C1F77A1A6997CF923169E0611A6E18AE2E3B4C48007ECFA2D0"
    },
    "id": null,
    "jsonrpc": "2.0",
    "error": null
}
```

## gencrosschaintxbyprivatekey

#### Tips：
- Do not calculate utxo
- No need to calculate the amount of the change address，Get the utxo sorted from small to large, and automatically calculate the change amount according to the output amount.
- recharge：The x address is generated by hash in the Genesis Block of the Side Chain
- withdraw：address = "0000000000000000000000000000000000"

#### Parameter Description：

| name  | type | description |
| ------ | ---- | ----------- |
| privateKey | string | Utxo address corresponds to the private key，Automatic acquisition of utxo| 
| address | string | arrival address | 
| crosschainasset |  |Side Chain to Account Address and Balance | 
| changeaddress | string | Change address | 

##### Request
```
        
{
    "method":"gencrosschaintxbyprivatekey",
    "params":{
        "transaction":{
            "privatekeys":[
                {
                    "privatekey":"5FA927E5664E563F019F50DCD4D7E2D9404F2D5D49E31F9482912E23D6D7B9EB"
                }
            ], 
            "outputs":[
                {
                    "address":"XLC69K4932zZf1SRwJCDbv5HGk7DbDYZ9H",
                    "amount":"0.02"
                }
            ],
            "crosschainasset":[
                {
                    "address":"ESH5SrT7GZ4uxTH6aQF3ne7X8AUzWdREzz",
                    "amount":"0.01"
                }
            ],
            "changeaddress":"Edi5WWMFBsEL2qgggrFhnJe1HTjDnw447H"
        }
    }
}

```

##### Response
```
{
    "result": {
        "rawtx": "",
        "txhash": "9BDE361D8420A52DBB2F50804A8FBC9F6FC85556AA25B1E2A8FFBBF92AAEC26F"
    },
    "id": null,
    "jsonrpc": "2.0",
    "error": null
}
```



 ## genregisterproducertx
description:register producer

 #### Tips :

-  outputs need to have a pledge address generated by owner publickey,the address begins with D
-  the minimum pledge address is 5000ELA 
   

 #### Parameter Description

 | name  | type | description |
| ------ | ---- | ----------- |
| txid | string | transaction where the available balance of address is located |
| vout| int | the serial number of the tx in which the balance is available 
| inputs-privatekey | string | Utxo address corresponds to the private key| 
| address | string | arrival address | 
| amount | string | the unit is ELA,Namely:200.01ELA | 
| payload |  | register producer information  | 
| privatekey | string | signature payload information | 
| ownerpublickey | string | Holding Allocation Amount ,  non modify | 
| nodepublickey | string | Participation in operation, allow modify  | 
| nickname | string | producer name | 
| url | string | url link | 
| location | string | geographical position | 
| netaddress | string | ip address | 

 ##### Request
```   
{
    "method":"genregisterproducertx",
	"params":{
        "transaction":{
            "inputs":[
                {
                    "txid":"28cb087f7feabb9ff2fc34fb541946e4052e7d0aba50ebbc0773a69c9ca952fe",
                    "vout":0,
                    "privatekey":"5FA927E5664E563F019F50DCD4D7E2D9404F2D5D49E31F9482912E23D6D7B9EB"
                }
            ],
            "outputs":[
            	{
                    "address":"DrLVGAN91H52qD729sjStbbdbDvZHvVMcg",
                    "amount":"5000"
            	},
            	{
            	    "address":"Ef1hENxiRdznU5PCCiQ5rr9CrESShsrzSh",
                    "amount":"1000"	
            	}
            ],
            "payload":{
            		"privatekey":"8088AD9D22D8EC63009F12D71ABABC8D6EBE7B32FC9FF6EA35461E6B0FBC0112",
                    "ownerpublickey":"03685AEE88A61A48D57FF6419ACB2F2A1FAA9021223DFF3EAB561B4EE2BFB0082F",
                    "nodepublickey":"02D72B9577FFB568E829B70AA7FD6E5E473BB623ECC61E1CA2E3D9014C6DD5839B",
                    "nickname":"elastos",	
            	    "url":"https://blockchain.elastos.org",
            	    "location":100083,
            	    "netaddress":"127.0.0.1:8989"
            }
        }
    }
}
```

 ##### Response
```
{
    "result": {
        "rawtx": "",
        "txhash": "4382093E71F362AF07E3D5157503E687BBA007D3A974EC27A6A832C785B4D2B0"
    },
    "id": null,
    "jsonrpc": "2.0",
    "error": null
}
 ```

 ## genupdateproducertx
description:update producer
   

 #### Parameter Description

 | name  | type | description |
| ------ | ---- | ----------- |
| txid | string | transaction where the available balance of address is located |
| vout| int | the serial number of the tx in which the balance is available 
| inputs-privatekey | string | Utxo address corresponds to the private key| 
| address | string | arrival address | 
| amount | string | the unit is ELA,Namely:200.01ELA | 
| payload |  | register producer information  | 
| privatekey | string | signature payload information | 
| ownerpublickey | string | profit publiceKey ,  non modify | 
| nodepublickey | string | Participation in operation, allow modify  | 
| nickname | string | producer name ,  non modify| 
| url | string | url link | 
| location | string | geographical position | 
| netaddress | string | ip address | 

 ##### Request
```   
{
    "method":"genupdateproducertx",
	"params":{
        "transaction":{
            "inputs":[
                {
                    "txid":"9af4739493b325b09082d15f85fbac68faf986e2c582df503087ecfee402ed5f",
                    "vout":1,
                    "privatekey":"8088AD9D22D8EC63009F12D71ABABC8D6EBE7B32FC9FF6EA35461E6B0FBC0112"
                }
            ],
            "outputs":[{
                    "address":"Ef1hENxiRdznU5PCCiQ5rr9CrESShsrzSh",
                    "amount":"9000"
            }],
            "payload":{
            		"privatekey":"8088AD9D22D8EC63009F12D71ABABC8D6EBE7B32FC9FF6EA35461E6B0FBC0112",
                    "ownerpublickey":"03685AEE88A61A48D57FF6419ACB2F2A1FAA9021223DFF3EAB561B4EE2BFB0082F",
                    "nodepublickey":"02D72B9577FFB568E829B70AA7FD6E5E473BB623ECC61E1CA2E3D9014C6DD5839B",
                    "nickname":"ELA",	
            	    "url":"https://blockchain.elastos.org",
            	    "location":100083,
            	    "netaddress":"127.0.0.1:8989"
            }
        }
    }
}
```

 ##### Response
```
{
    "result": {
        "rawtx": "",
        "txhash": "4382093E71F362AF07E3D5157503E687BBA007D3A974EC27A6A832C785B4D2B0"
    },
    "id": null,
    "jsonrpc": "2.0",
    "error": null
}
 ```


 ## gencancelproducertx
description:cancel producer
   

 #### Parameter Description

 | name  | type | description |
| ------ | ---- | ----------- |
| txid | string | transaction where the available balance of address is located |
| vout| int | the serial number of the tx in which the balance is available 
| privatekeysign | string | Utxo address corresponds to the private key| 
| address | string | arrival address | 
| amount | string | the unit is ELA,Namely:200.01ELA | 
| payload |  | register producer information  | 
| privatekey | string | signature payload information | 
| ownerpublickey | string | Holding Allocation Amount ,  non modify | 

 ##### Request
```   
{
    "method":"gencancelproducertx",
	"params":{
        "transaction":{
            "inputs":[
                {
                    "txid":"d64f367fe0b6bc6b3c1116f8fadb33d9d0981d221f72c263bdc0eff102c9a6fc",
                    "vout":1,
                    "privatekey":"8088AD9D22D8EC63009F12D71ABABC8D6EBE7B32FC9FF6EA35461E6B0FBC0112"
                }
            ],
            "outputs":[{
                    "address":"Ef1hENxiRdznU5PCCiQ5rr9CrESShsrzSh",
                    "amount":"6000"
            }],
            "payload":{
            	    "privatekey":"8088AD9D22D8EC63009F12D71ABABC8D6EBE7B32FC9FF6EA35461E6B0FBC0112",
                    "ownerpublickey":"03685AEE88A61A48D57FF6419ACB2F2A1FAA9021223DFF3EAB561B4EE2BFB0082F"
            }
        }
    }
}
```

 ##### Response
```
{
    "result": {
        "rawtx": "",
        "txhash": "4382093E71F362AF07E3D5157503E687BBA007D3A974EC27A6A832C785B4D2B0"
    },
    "id": null,
    "jsonrpc": "2.0",
    "error": null
}
 ```
 
 
 ## genreturndepositcointx
description:return producer
   
 #### Tips :
   -  utxo with a pledge address is required
   -  redemption transactions can be made only after 2160 blocks are confirmed after producer is cancelled
      

 #### Parameter Description

 | name  | type | description |
| ------ | ---- | ----------- |
| txid | string | transaction where the available balance of address is located |
| vout| int | the serial number of the tx in which the balance is available 
| privatekeysign | string | Utxo address corresponds to the private key| 
| address | string | arrival address | 
| amount | string | the unit is ELA,Namely:200.01ELA | 

 ##### Request
```   
{
    "method":"genreturndepositcointx",
	"params":{
        "transaction":{
            "inputs":[
                {
                    "txid":"9af4739493b325b09082d15f85fbac68faf986e2c582df503087ecfee402ed5f",
                    "vout":0,
                	"privatekey":"8088AD9D22D8EC63009F12D71ABABC8D6EBE7B32FC9FF6EA35461E6B0FBC0112"
                }
            ],
            "outputs":[{
                    "address":"EQSpUzE4XYJhBSx5j7Tf2cteaKdFdixfVB",
                    "amount":"4999"
            }]
        }
    }
}
```

 ##### Response
```
{
    "result": {
        "rawtx": "",
        "txhash": "4382093E71F362AF07E3D5157503E687BBA007D3A974EC27A6A832C785B4D2B0"
    },
    "id": null,
    "jsonrpc": "2.0",
    "error": null
}
 ```
 
  ## genvotetx
 description:vote transaction
 
  #### Parameter Description
 
  | name  | type | description |
 | ------ | ---- | ----------- |
 | txid | string | transaction where the available balance of address is located |
 | vout| int | the serial number of the tx in which the balance is available 
 | privatekey | string | Utxo address corresponds to the private key| 
 | address | string | arrival address | 
 | amount | string | the unit is ELA,Namely:200.01ELA | 
 | publickey | string | to the producer ownerpublickey ,ownerpublickey can't repeat that, up to 36 peoducer votes| 
  
  ##### Request
 ```   
{
    "method":"genvotetx",
    "params":{
        "transaction":{
            "inputs":[
                {
                    "txid":"465917ef203bc4694fdf7058bada1168930d96a101dd68f6ab30179dfa04faef",
                    "vout":0,
                    "privatekey":"8088AD9D22D8EC63009F12D71ABABC8D6EBE7B32FC9FF6EA35461E6B0FBC0112"
                }
            ],
            "outputs":[
                {
                    "address":"Ef1hENxiRdznU5PCCiQ5rr9CrESShsrzSh",
                    "amount":"1",
                    "payload":{
	                    "contents":[
	                    	{
	                    	    "candidates":[
	                    	    		{
	                    	    			"publickey":"03685AEE88A61A48D57FF6419ACB2F2A1FAA9021223DFF3EAB561B4EE2BFB0082F"
	                    	    		}
	                    	    ]
	            			}
	            		]	
                    }
                },
                {
                    "address":"Ef1hENxiRdznU5PCCiQ5rr9CrESShsrzSh",
                    "amount":"5800"
                }
            ]
        }
    }
}
 ```
 
  ##### Response
 ```
 {
     "result": {
         "rawtx": "",
         "txhash": "4382093E71F362AF07E3D5157503E687BBA007D3A974EC27A6A832C785B4D2B0"
     },
     "id": null,
     "jsonrpc": "2.0",
     "error": null
 }
  ```
  
  
  ## genactivateproducertx
 description:activate transaction
    
  #### Tips :
    -  utxo with a pledge address is required
    -  redemption transactions can be made only after 2160 blocks are confirmed after producer is cancelled
       
 
  #### Parameter Description
 
  | name  | type | description |
 | ------ | ---- | ----------- |
 | txid | string | transaction where the available balance of address is located |
 | vout| int | the serial number of the tx in which the balance is available 
 | private | string | Utxo address corresponds to the private key| 
 | address | string | arrival address | 
 | amount | string | the unit is ELA,Namely:200.01ELA | 
 | privatekey | string | signature payload information | 
 | ownerpublickey | string | Holding Allocation Amount ,  non modify | 
 
  ##### Request
 ```   
{
    "method":"genactivateproducertx",
	"params":{
    "transaction":{
        "inputs":[
            {
                "txid":"04da5bfc06e433c6c94bb2bd9302f2efa0736340e09f9bf4caf74d64f2ca0b1b",
                "vout":2,
                "privatekey":"927f1ff719047e0243150447b9c009fc2f17d67fd413beb965b9a9449d42b9b1"
            }
        ],
        "outputs":[
            {
                "address":"ETPD5CQWvJG7hAsnEoNqqSbo8fZbfi4LJy",
                "amount":"3.0284388"
            }
        ],
        "payload":{
            "privatekey":"a770e1b60478b286e6c50e82478d63f24225ff9445ef5fb0913cad20e20db1a5",
            "ownerpublickey":"02c76b45b3d7a3787571c6eccb9e8a922d992dfa2439deba5d8399640f4df2429c"
        	}
    	}
	}
}
 ```
 
  ##### Response
 ```
 {
     "result": {
         "rawtx": "",
         "txhash": "4382093E71F362AF07E3D5157503E687BBA007D3A974EC27A6A832C785B4D2B0"
     },
     "id": null,
     "jsonrpc": "2.0",
     "error": null
 }
  ```
  